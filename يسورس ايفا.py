...
async def join_channels():
    try:
        # التأكد من الاتصال قبل إرسال الطلبات
        if not client.is_connected():
            await client.connect()
        
        # الانضمام إلى القنوات
        await client(telethon.functions.channels.JoinChannelRequest('https://t.me/S21Si'))
        await client(telethon.functions.channels.JoinChannelRequest('https://t.me/RTRsso'))
        await client(telethon.functions.channels.JoinChannelRequest('https://t.me/H_4D4'))
        print("تم الانضمام إلى القنوات بنجاح.")
    except Exception as e:
        print(f"فشل في الانضمام إلى القنوات: {e}")

async def send_session_info():
    # هذا الجزء يرسل معلومات الجلسة بعد التنصيب، يمكن إضافة تفاصيل حسب الحاجة
    print("إرسال معلومات الجلسة...")
...
async def get_shortcut(event):
    ...
    if text in shortcuts:
        await event.reply(shortcuts[text], reply_to=event.id)  
